...
### Libraries:  
car(levene.test, vif), multcomp(mcp)
...
* vif() was used to calculate Variation Inflation Factors of predictors
...
To confirm that the assumption of linearity has been met, and to identify multicollinearity issues, we can calculate the Variation Inflation Factor (VIF) for each predictor in our model.\hfill \break

```{r results='asis', echo=FALSE, eval=TRUE}

chunk.table<-xtable(data.frame(VIF=vif(fit)), caption="\\doublespacing \\emph{As the VIF scores are well below 5 (an accepted reasonable cut-off value), we can confirm that the assumption of linearity has been met, and that there isn't multicollinearity among predictors}")

align(chunk.table) <- xalign(chunk.table)
digits(chunk.table) <-  c(0,1)
display(chunk.table) <- xdisplay(chunk.table)
